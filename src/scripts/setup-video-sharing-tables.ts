...
export async function setupVideoSharingTables() {
  try {
    logInfo('Setting up video sharing and analytics tables...');

    // Create video_shares table
    const { error: videoSharesError } = await supabase.rpc('exec_sql', {
      sql: `
    ...
    const { error: analyticsEventsError } = await supabase.rpc('exec_sql', {
      sql: `
    ...
    const { error: analyticsFunctionError } = await supabase.rpc('exec_sql', {
      sql: `
    ...
    const { error: addProviderColumnError } = await supabase.rpc('exec_sql', {
      sql: `
        DO $$ 
        BEGIN
          IF NOT EXISTS (
    ...
    const { error: addErrorMessageColumnError } = await supabase.rpc('exec_sql', {
      sql: `
        DO $$ 
        BEGIN
          IF NOT EXISTS (
    ...
    logInfo('Video sharing and analytics tables setup completed successfully');

  } catch (error) {
    logError(error as Error, 'Failed to setup video sharing tables');
    throw error;
  }
}
