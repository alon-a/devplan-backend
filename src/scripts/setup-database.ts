...
const createTables = async () => {
  try {
    ...
    const { error: dialoguesError } = await supabase.rpc('exec_sql', {
      sql: `
    ...
    const { error: insertTemplatesError } = await supabase.rpc('exec_sql', {
      sql: `
    ...
    if (insertTemplatesError) {
      logError(insertTemplatesError, 'Failed to insert default templates');
    }
